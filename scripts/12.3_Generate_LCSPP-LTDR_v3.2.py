...
def generate_lcspp_prediction(file, ssrd_file):
    ...
    lcspp_ds.attrs = {"title": "Semi-Monthly Global Reconstruction LCSPP using LTDR", 
                    ...
                    "date_source": "LCREF_v3.2, MCD43C1 v061, OCO-2 SIF Lite V11r, ERA5 Reanalysis",
                    "created_date":datetime.date.today().strftime("%m/%d/%Y")}    
    
    lcspp_ds.to_netcdf(os.path.join(LCSPP_v3_DIR, "LCSPP" + file.split("/")[-1][5:]))
    lcspp_ds.close()
    ds.close()
...
lcspp_train_mean = np.load("../../data/sif_train_mean.npy")
lcspp_train_var = np.load("../../data/sif_train_var.npy")
lcspp_train_scale = np.load("../../data/sif_train_scale.npy")
