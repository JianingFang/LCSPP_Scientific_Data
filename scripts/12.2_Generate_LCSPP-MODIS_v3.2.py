...
LCSPP_v3_DIR = "../../data/MODIS_LCSPP_v3/"
...
def generate_lcspp_prediction(file, ssrd_file):
    ...
    DOI = "10.5281/zenodo.11658088"

    lcspp_ds.attrs = {"title": "SIF-informed Photosynthesis Proxy reconstructed with MODIS surface reflectance (LCSPP-MODIS)", 
                    ...
                    "date_source": "LCREF_MODIS_v3.2, OCO-2 SIF Lite V11r, ERA5 Reanalysis",
                    "created_date":datetime.date.today().strftime("%m/%d/%Y")}    
    
    lcspp_ds.to_netcdf(os.path.join(LCSPP_v3_DIR, "LCSPP_MODIS_v3.2_" + file.split("/")[-1][5:]))
    lcspp_ds.close()
    ds.close()
...
SSRD_YEAR_LIST = np.array(sorted([os.path.join("../../../data/ERA5", f) for f in os.listdir("../../../data/ERA5") if "CMG_ssrd_biweekly_" in f])).reshape(-1, 24)[19 + IDX]

for l in range(len(AVHRR_FILE_YEAR_LIST_AVAILABLE)):
    generate_lcspp_prediction(AVHRR_FILE_YEAR_LIST_AVAILABLE[l], SSRD_YEAR_LIST[l])
