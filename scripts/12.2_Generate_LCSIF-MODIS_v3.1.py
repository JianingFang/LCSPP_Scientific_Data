...
LCREF_v3_DIR = "../../data/MODIS_LCREF_v3/"
LCSIF_v3_DIR = "../../data/MODIS_LCSIF_v3/"
...
def generate_sif_prediction(file, ssrd_file):
    ...
    sif_ds["sif_all_daily"].attrs={"long_name":"all-sky daily average reconstructed SIF proxy weighted by ERA5 SSRD", "units":"mW m-2 nm-1 sr-1"}
    sif_ds["sif_clear_inst"].attrs={"long_name":"instantaneous clear-sky reconstructed SIF proxy", "units":"mW m-2 nm-1 sr-1"}
    sif_ds["sif_clear_daily"].attrs={"long_name": "daily clear-sky reconstructed SIF proxy adjusted by cosine solar zenith angle", "units":"mW m-2 nm-1 sr-1"}
    sif_ds = sif_ds.astype(np.float32)    
    sif_ds.attrs = {"title": "A Reconstruction of Long-Term Spatially Contiguous Solar-Induced Fluorescence Proxy from Calibrated MODIS Surface Reflectance (LCSIF-MODIS)", 
                    ...
                    "date_source": "LCREF_MODIS_v3.1, OCO-2 SIF Lite V11r, ERA5 Reanalysis",
...
time_arr = np.load("../../data/time_arr.npy").reshape(-1, 24)

IDX = int(os.getenv('SLURM_ARRAY_TASK_ID')) - 1
AVHRR_FILE_YEAR_LIST_AVAILABLE = np.array([os.path.join(LCREF_v3_DIR, f) for f in sorted(os.listdir(LCREF_v3_DIR)) if "SNOW" in f]).reshape(-1, 24)[IDX]
SSRD_YEAR_LIST = np.array(sorted([os.path.join("../../../data/ERA5", f) for f in os.listdir("../../../data/ERA5") if "CMG_ssrd_biweekly_" in f])[0:-10]).reshape(-1, 24)[19 + IDX]
