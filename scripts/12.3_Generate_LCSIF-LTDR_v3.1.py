...
LCREF_v3_DIR = "../../data/LTDR_LCREF/"
LCSIF_v3_DIR = "../../data/LTDR_LCSIF/"
...
def generate_sif_prediction(file, ssrd_file):
    ...
    sif_ds["sif_clear_inst"].attrs={"long_name":"instantaneous clear-sky predicted SIF", "units":"mW m-2 nm-1 sr-1"}
    sif_ds["sif_clear_daily"].attrs={"long_name": "daily clear-sky predicted SIF adjusted by cosine solar zenith angle", "units":"mW m-2 nm-1 sr-1"}
    sif_ds = sif_ds.astype(np.float32)    
    sif_ds.attrs = {"title": "Semi-Monthly Global Reconstruction LCSIF using LCREF", 
                    ...
                    "date_source": "LCREF_v2, MCD43C1 v061, OCO-2 SIF Lite V10, ERA5 Reanalysis",
                    "created_date":datetime.date.today().strftime("%m/%d/%Y")}    
    
    sif_ds.to_netcdf(os.path.join(LCSIF_v3_DIR, "LCSIF" + file.split("/")[-1][5:]))
    sif_ds.close()
    ds.close()
...
sif_train_mean = np.load("../../data/sif_train_mean.npy")
sif_train_var = np.load("../../data/sif_train_var.npy")
sif_train_scale = np.load("../../data/sif_train_scale.npy")
...
model_name="layer_2_neuron_64_08-08-2023_01-40-17_lr0.001_batchsize1024"
