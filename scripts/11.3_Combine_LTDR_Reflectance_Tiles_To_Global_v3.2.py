...
GAPFILLED = "AVHRR/data/LTDR_GAPFILLED/"
LCREF = "AVHRR/data/LTDR_LCREF/"
...
for i in tqdm(range(len(yearmonth_list))):
    ...
    gap_fill_nir_parts = [os.path.join(GAPFILLED, f) for f in gap_fill_list if yearmonth_list[i] in f and "nir" in f]
    gap_fill_red_parts = [os.path.join(GAPFILLED, f) for f in gap_fill_list if yearmonth_list[i] in f and "red" in f]
    ...
    lcref_ds.attrs = {"title": "Long-term Continuous REFlectance (LCREF), LTDR", 
    ...
    lcref_ds.to_netcdf(os.path.join(LCREF, "LCREF_LTDR_{}.nc".format(yearmonth_list[i])))
